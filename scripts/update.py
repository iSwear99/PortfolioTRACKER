...
Spouští GitHub Actions 2x denně. Lze pustit i ručně:
    TWELVE_DATA_KEY=xxx python scripts/update.py
Bez klíče (nebo při chybě API) se použijí poslední známé ceny — tracker
se vždy vygeneruje, jen s příznakem stáří dat.
"""
...
def fetch_cnb_fx(old_fx):
    ...
    Formát: zeme|mena|mnozstvi|kod|kurz  (kurz s čárkou)."""
...
def fetch_prices(seed):
    ...
    for t, cfg in prices.items():
        try:
            ...
            data = json.loads(http_get('https://api.twelvedata.com/price?' + urllib.parse.urlencode(params)))
            px = float(data['price'])
            if cfg.get('divide'): px /= cfg['divide']   # LSE kotace v GBX -> GBP
            if px > 0:
...
def xirr(flows, nav, asof):
    """flows: [(date_str, czk_amount_signed_from_investor_view)] + final NAV."""
