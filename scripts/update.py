# -*- coding: utf-8 -*-
"""
Aktualizace portfolia: ceny (Twelve Data + záloha Stooq) a kurzy (ČNB).
"""
...
def http_get(url, timeout=25):
    req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0 portfolio-tracker'})
...
def fetch_prices(seed):
    ...
    fetched, failed = [], []
    if not key:
        print('[WARN] TWELVE_DATA_KEY není nastaven.', file=sys.stderr)
        return prices, fetched, list(prices)
    for i, (t, cfg) in enumerate(prices.items()):
        if i: time.sleep(8.5)               # free tier: 8 dotazů/min
        for attempt in (1, 2):
            try:
...
# ---------- Stooq záloha (bez klíče) ----------
STOOQ = {'BOSS': ('boss.de', 1), 'P911': ('p911.de', 1), 'NOV': ('nov.de', 1),
         'VOW3': ('vow3.de', 1), 'CSPX': ('cspx.uk', 1), 'WIZZ': ('wizz.uk', 100),
         'EVO': ('evo.se', 1)}

def fetch_stooq(prices, failed):
    recovered = []
    for t in list(failed):
        sym, div = STOOQ.get(t, (t.lower() + '.us', 1))
        try:
            time.sleep(1.2)
            csv = http_get(f'https://stooq.com/q/l/?s={sym}&f=sd2t2ohlcv&h&e=csv')
            row = csv.strip().split('\n')[1].split(',')
            px = float(row[6]) / div          # sloupec Close
            last = prices[t]['px']
            if px > 0 and (last <= 0 or 0.33 < px / last < 3):
                prices[t]['px'] = round(px, 4)
                recovered.append(t); failed.remove(t)
            else:
                print(f'[WARN] Stooq {t}: cena {px} neprošla kontrolou vs {last}', file=sys.stderr)
        except Exception as e:
            print(f'[WARN] Stooq {t} ({sym}): {e}', file=sys.stderr)
    return recovered
...
def main():
    ...
    if rec: note += f'; {len(rec)} ze Stooq ({", ".join(rec)})'
